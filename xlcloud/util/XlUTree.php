...
class XlUTree {
        ...
        foreach ($path as $i){
            ...
            if(!array_key_exists($i, $next)){
                ...
                if(!array_key_exists(self::$wildcard, $next)){
                    //正则匹配
                    foreach ($next as $ki=>$ni){

                        if(preg_match("/(\(([^\/]+)\))/",$ki,$match)){

                            $regstr=$match[count($match)-1];

                            if(preg_match("/^".$regstr."$/",$i,$match)){
                                //成功匹配
                                $isexsit=true;
                                $pos=&$next[$ki];
                                $this->attachNodeMatchValue($pos,$match);
    ...
    private function attachNodeMatchValue(&$pos,$match){

        if($pos['next']&&is_array($pos['next'])){
            foreach ($pos['next'] as &$pn){
                if($pn['value']&&is_array($pn['value'])){
                    $param=$pn['value']['param'];
                    if($param&&is_array($param)){
                        $pn['value']['param']=array_merge($param,$match);
                    }else{
                        $pn['value']['param']=$match;
                    }
                }else{
                    if(is_array($pn)){
                        $this->attachNodeMatchValue($pn,$match);
                    }
                }
            }
        }
