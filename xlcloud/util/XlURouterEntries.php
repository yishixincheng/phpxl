 ...
 */
class XlURouterEntries{

    private $routes;
    function __construct(){
        $this->routes = new XlUTree();
    }
    /**
     *
     * @param string $q
    ...
    function findByArray($paths,$params, &$matched_path =null){
        ...
        array_unshift($paths, '/');
        $paths[] = XlUTree::$end;  //最后总是插入Tree::$end表示路径结束
        ...
        $this->routes->visit($paths,function ($node,$value)use(&$valid_path,&$route,&$visited,&$geted,&$walked_path){
            ...
            if(!is_null($value)) {
                $route=$value;
                if(!($route instanceof \xl\util\XlUTree)) {
                    $valid_path=$value;
                }
                $geted = $visited;
    ...
    public function insert($query,$e,$strict=false){
        ...
        $ismt=false;
        $path=preg_replace_callback("/(\([^\/]+\))/",function($match) use(&$ismt){
            $ismt=true;
        ...
        if($strict || !empty($params)){
            $paths[]=XlUTree::$end; //对于严格匹配的路由规则（有querystring的路由，path部分也需要严格匹配），在路径末尾加入end，偏于匹配
        }
    ...
    public function insertByArray($paths,$params,$e){
        ...
        if( $params !== null){
            ...
            if($node && $node['value'] instanceof  \xl\util\XlUTree){
                $route = $node['value'];
    ...
    public function export(){
        ...
        foreach ($ori as $v){
            list($path, $value) = $v;
            if($value instanceof \xl\util\XlUTree){
