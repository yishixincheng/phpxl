 ...
 *
 */
class WxPayTransferToWallet extends WxPayDataBase
{
    /**
    ...
    /**
     * 校验用户姓名选项NO_CHECK：不校验真实姓名FORCE_CHECK：强校验真实姓名
     */
    public function SetCheck_name($value){

        return $this->values['check_name']=$value;
...
/**
 *
 * 用于对商户企业付款到零钱操作进行结果查询，返回付款操作详细结果。
 * @author 一世心城
 *
 */
class WxPayTransferToWalletResultQuery extends WxPayDataBase
{
 ...
 *
 */
class WxPayTransferToBank extends WxPayDataBase
{
    ...
    /**
     * 收款方银行卡号收款方银行卡号（采用标准RSA算法，公钥由微信侧提供）,详见获取RSA加密公钥API
     */
    ...
    /**
     * 收款方用户名收款方用户名（采用标准RSA算法，公钥由微信侧提供）详见获取RSA加密公钥API
     */
    ...
    public function IsBank_codeSet(){
 ...
 *
 */
class WxPayTransferToBankResultQuery extends WxPayDataBase
{
 ...
 *
 */
class WxPayGetRSAPublicKey extends WxPayDataBase
{
 ...
 *
 */
class WxPayResultEx extends WxPayDataBase
{
    /**
     *
     * 检测签名
     ...
     * @throws WxPayException
     */
    public static function Init($xml,$noCheckSign=false)
    {
        $obj = new self();
        $obj->FromXml($xml);
        //fix bug 2015-06-29
