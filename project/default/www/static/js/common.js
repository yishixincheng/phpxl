/**
 * Created by TaiGuangYin on 2015/1/15.
 */
...
commonUtil.dateFormat = function (date,fmt) {
    if(!date){
        date = new Date();
    }
    if(!fmt){
 ...
 * 获取当前月的第一天
 */
commonUtil.getCurrentMonthFirst = function(){
    var date=new Date();
    date.setDate(1);
    return date;
}
 ...
 * 获取当前月的最后一天
 */
 commonUtil.getCurrentMonthLast = function(){
    var date=new Date();
